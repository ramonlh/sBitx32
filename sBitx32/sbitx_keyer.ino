...
void cwKeydown(){
  ...
  cwTimeout = millis() + conf.cwDelayTime * 10;  
}
...
char update_PaddleLatch(byte isUpdateKeyState) {
  ...
  if (paddle >= conf.cwAdcDashFrom && paddle <= conf.cwAdcDashTo)
    ...
    {
    if (conf.cwKeyType>0)   // Iambic_Key
      tmpKeyerControl = 0 ;
        ...
        case KEYED_PREP:
          ...
          */
          if (!inTx){
            ...
            keyDown = 0;
            cwTimeout = millis() + conf.cwDelayTime * 10;  //+ CW_TIMEOUT;
            startTx(TX_CW, 1);
          }
          ktimer += millis(); // set ktimer to interval end time
          keyerControl &= ~(DIT_L + DAH_L); // clear both paddle latch bits
    ...
    while(1)
      ...
      if (state == DIT_L) 
        ...
        if (!inTx)
          {
          //DelayTime Option
          delay_background(conf.delayBeforeCWStartTime*2, 2,0);
          keyDown = 0;
          cwTimeout = millis() + conf.cwDelayTime * 10;  //+ CW_TIMEOUT; 
          startTx(TX_CW, 1);
          }
        ...
        while (update_PaddleLatch(0) == DIT_L ) 
          { 
          delay_background(conf.cwDelayTime,3,1); 
          }
        cwKeyUp();
        }
      else
        {
        if ((cwTimeout > 0) && (cwTimeout < millis()))
          {
          cwTimeout = 0;
          keyDown = 0;
          stopTx();
